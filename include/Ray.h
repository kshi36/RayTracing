...
Ray RayTracer::RayThruPixel(Camera *cam, int i, int j, int width, int height) {
    ...
    Ray ray;
    ray.p0 = cam->eye;
    ray.dir = glm::normalize(alpha*cam->aspect*glm::tan((cam->fovy)/2)*u
...
//        intersect.dist = std::numeric_limits<float>::infinity();
...
//            std::cout << "new mindist = " << mindist << std::endl;
    ...
    std::cout << "Intersect dist2: " << hit.dist << std::endl;
...
//    std::cout << "test inf: " << std::isinf(std::numeric_limits<float>::infinity()) << std::endl;
    float testINF = MY_INFINITY;
//    std::cout << "test inf constant: " << (compare_float(testINF, MY_INFINITY)) << " (should be 1?)" << std::endl;
    //TODO: check if intersection exists
    if (compare_float(hit.dist, MY_INFINITY)) {     //intersection doesn't exist (infinity)
